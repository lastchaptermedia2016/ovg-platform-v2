...
export const FEATURE_REGISTRY: Record<SYSTEM_COMMAND, FeatureRegistryEntry> = {
  ...
  SYSTEM_UPDATE_PERSONA: {
    ...
    scope: 'client',
    uiModal: 'src/components/client/AIPersonaSettings.tsx',
    handler: 'AIPersonaSettings',
    requiresAuth: true,
    description: 'Update AI assistant persona: name, greeting, voice, and system instructions',
  },
  SYSTEM_MANAGE_MEMORY: {
    actionType: 'SYSTEM_MANAGE_MEMORY',
    scope: 'client',
    uiModal: 'src/components/studio/ClientKnowledgeManager.tsx',
    handler: 'ClientKnowledgeManager',
    requiresAuth: true,
  ...
  SYSTEM_PUBLISH_DRAFT: {
    ...
    scope: 'client',
    uiModal: 'src/components/client/BrandingStudio.tsx',
    handler: 'BrandingStudio',
    requiresAuth: true,
    description: 'Commit the current unsaved studio draft to live tenant configuration',
  },
