...
export default async function TenantRegistryTable() {
  ...
  const { data: resellers, error } = await supabase
    .from('resellers')
    .select('id, tenant_id, name, owner_email, email, branding_color, accent_color, logo_url, is_active, created_at, updated_at, version_stamp, branding_bag')
  ...
  const records: ResellerRecord[] = (resellers ?? []).map((r) => ({
    ...r,
    branding_bag: (r as ResellerRecord).branding_bag ?? {
      primaryColor: r.branding_color || '#0097b2',
